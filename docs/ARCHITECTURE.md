...
| Terugkerende maandkosten: sjabloon-beheer + maandpost genereren (idempotent per sjabloon+maand) | `app/services/terugkerend.doge` — bouwt de maandpost via `journaal.inkoop_post` (btw-splitsing hangt aan de `btw_code`); getriggerd door de knop (`terugkerend_h`) én door de scheduler-pup (`intern_h.run_recurring`, §5.2) |
| De drie terugkerende stappen samen draaien (scheduler + handmatig) | `app/handlers/intern_h.doge` `run_recurring` — token-guard (`crypto.same` op `INTERN_TOKEN`) + `mollie.sync` + `terugkerend.genereer` + `bijlagen.scan_import`, elk idempotent; de scheduler-pup in `main.doge` POSTt hier dagelijks (`datum.seconden_tot_uur` + `nap.rest`) |
| Bijlagen: upload-validatie (ext-allowlist + size-cap), opslag `uploads/{jaar}/{id}{ext}`, import-inbox, `data/import/`-scan, koppelen | `app/services/bijlagen.doge` — owns de allow-list/size-cap/mime-constanten; `scan_import` leest de import-map via store (`import_lijst`/`import_verwijder`) en boekt elk bestand via `bewaar(..., "import")`, bronfile daarna verwijderd (idempotent); roept `journaal.koppel_bijlage` voor de post-kant (single writer) |
...
Browser (LAN/VPN) ──HTTP──▶ VM ▸ Docker container ▸ boekhond binary
                              ...
                              (sequential)               sleeps until 06:00,
                                    ...
                                    ▼                    via howl.request loopback
                          web/http.doge parse            (Mollie-sync + terugkerende
                                    ▼                     kosten + scan data/import/)
...
**There is no login and no wachtwoord.** Single-user app; the only access boundary is the network — LAN/VPN-only (decided 2026-07-15), meant to sit behind a reverse proxy. An application password was deliberately dropped: with the plaintext already on the same host and no TLS in-app, a fast unsalted hash added no real protection, and Doge's `crypto` has no KDF (only `sha256`/`hmac`/`token`/`same`) to do it properly. Every route is served directly; `web/http.doge` still parses cookies generically, but nothing gates on them. `/internal/*` guards on `INTERN_TOKEN` (constant-time `crypto.same`) — service-to-service auth, not user login. Loopback-peer filtering was intended as defense-in-depth but **howl v0.3.3 exposes no peer address** (`accept()→Socket`, geen remote addr in de request-Dict) → doge#71, token-only als stopgap; de reverse-proxy hoort `/internal/` extern te blokkeren. Bij de upgrade naar v0.3.4 (waar doge#71 landt) komt de peer-check erbij.

### 5.2 Scheduler (Mollie-sync + terugkerende kosten)

A single pup (`pack.zoom(scheduler_lus, [cfg])`) started in `main.doge` `start()` (only when `INTERN_TOKEN` is set): loop { `nap.rest(datum.seconden_tot_uur(6))` (sleep to the next 06:00; timing is pure/tested via `seconden_tot_uur_vanaf`), `howl.request("POST", ...)` loopback with an `X-Intern-Token` header, log the status }. The handler `app/handlers/intern_h.doge` `run_recurring` runs on the main loop (so it *does* share the store), first guards the token with `crypto.same` (403 on missing/wrong), then does three things — each in its own `pls`/`oh no` so one failure doesn't stop the rest — all **idempotent**: (1) `mollie.sync(store, key)` (skipped when no key) calls the Mollie API (`howl.request`, `MOLLIE_API_KEY`) for paid payments + refunds since `instellingen.mollie_laatste_sync`, boekt elke als omzet-post (idempotent per `mollie_payment_id`), advances the hoogwatermerk; (2) `terugkerend.genereer(store, datum.vandaag())` walks `terugkerend.dson`, generates due kosten-journaalposten (idempotent per sjabloon+maand); (3) `bijlagen.scan_import(store)` scans the mounted `data/import/` map (via de store-methods `import_lijst`/`import_verwijder`) — every new file becomes a Bijlage (`bron = "import"`) in the inbox via `bijlagen.bewaar` (which already copies the bytes to `uploads/{jaar}/{id}.{ext}` + audit), and the **source file in `data/import/` is then deleted** (so import never double-ingests — idempotency is by source-file removal, not a stored key). An invalid file (ext/size) stays put and is reported skipped. Each step is ook los met de hand te draaien (`POST /mollie/sync`, `/terugkerend/genereer`, `/inbox/scan`). `/internal/*` accepts **only the token** — loopback-peer filtering is niet mogelijk in howl v0.3.3 (doge#71); de reverse-proxy blokkeert `/internal/` extern. Pups share no state (§6): loopback HTTP is the only correct channel; never give the pup its own store access.
...
`.env` read once at boot → config Dict → passed down explicitly. `main.doge` loads `.env` itself (a small parser; real environment variables win, so `docker compose env_file`/exports override the file). Keys: `INTERN_TOKEN` (none → scheduler-pup niet gestart, `/internal/*` weigert alles), `MOLLIE_API_KEY`, `POORT`, `DATA_DIR`, `BIND_HOST` (listener-bind; default `127.0.0.1`, in de container zet `docker-compose.yml` dit op `0.0.0.0` zodat de gepubliceerde poort/reverse-proxy hem bereikt — `main.doge` bindt `cfg["bind_host"]`). Bedrijfsgegevens (naam, KvK, btw-id, IBAN, adres) are *not* secrets and live in `instellingen.dson`, editable in the UI.
...
1. **Pups (threads) deep-copy everything and share no mutable state.** A shared in-memory store across threads is impossible by design → sequential accept-loop + loopback HTTP for the scheduler. Bowls/sockets are the only shared handles. A pup *does* see the top-level snapshot (imported modules incl. `nap`/`howl`/`datum` are usable inside `scheduler_lus` — verified). **`howl.accept` exposes no peer address** (doge#71) → `/internal/*` is token-only (`crypto.same`), not loopback-peer-filtered; landt in v0.3.4.
